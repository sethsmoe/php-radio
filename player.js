...
// THIS IS A FORK OF SOMETHING!
// I cant actually find the original sourcecode, no .git/ either.

var track,audio,audioUI,pic,title,ID3,shuffle,repeat,loop,err,playlist,tabs,unPlayed,trackAction,
	log=false, // shows messages in browser console
	music=Array(),
	spam={
		"block":false,
		"lock":function(){
...
function sendEvt(element,event){
	...
	//element.dispatchEvent(new Event(event));//This does not work in IE11, but who uses anything other than firefox / spyware
...
function play(){
	...
	if(promise!==undefined){
		promise.then(function(){
			...
			console.warn('Audio playback was prevented by browser.');// Note that using => breaks IE
			sendEvt(audio,'pause');
		});
	}
}
		...
		if(i!="/"){
			...
			li.addEventListener('dblclick',function(event){
				...
				if(confirm('Only play from:\n\t'+p)){
					// Yes, it is possible to accomplish this via JavaScript, but this was quick and easy implementation for a feature I dont care about
...
function init(){
	...
	if(path.length>2){
		...
		li.textContent=path==''?'<<< BACK TO LIST':path;
		ul.appendChild(li);
		li.addEventListener('click',function(event){
			event.stopPropagation();
			var p=this.getAttribute('path');
	...
	if(audioUI){
		// i dont use this crap, so it might work ¯\_(ツ)_/¯
		audioUI={
			"ui":audioUI,
